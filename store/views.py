...
from .models import Cart, CartItem, Collection, Customer, Order, ProductImages, Product, Review, Store
from .serializers import AddCartItemSerializer, CartItemSerializer, CartSerializer, CollectionSerializer, \
    CreateOrderSerializer, CustomerSerializer, OrderSerializer, ProductImageSerializers, ProductSerializer, \
    ReviewSerializer, UpdateCartItemSerializer, UpdateOrderService, StoreListSerializer, StoreDetailSerializer, \
    CreateStoreSerializer, CreateProductSerializer
...
class ProductViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            return Product.objects.select_related("store").prefetch_related('images').filter(store_id=store_id)
        except Store.DoesNotExist:
            Product.objects.prefetch_related('images').all()


class StoreViewSet(ModelViewSet):
    queryset = Store.objects.select_related('products').select_related('user').select_related('address').all()
    filter_backends = [DjangoFilterBackend, SearchFilter, OrderingFilter]
    ...
    def me(self, request):
        ...
        if request.method == 'GET':
            ...
            store = Store.objects.get(user_id=request.user.id)
            serializer = StoreDetailSerializer(store, request.data)
            serializer.is_valid(raise_exception=True)
            serializer.save()
            return Response(serializer.data)
        elif request.method == "POST":
            serializer = CreateStoreSerializer(
                data=request.data,
                context={'user_id': self.request.user.id})
            serializer.is_valid(raise_exception=True)
